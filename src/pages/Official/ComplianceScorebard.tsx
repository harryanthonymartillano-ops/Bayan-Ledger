...
export const ComplianceScorebardPage: React.FC = () => {
  const { projects, calculateComplianceScore } = useBlockchain();

  const complianceScores = useMemo(() => {
    const scores = new Map();
    projects.forEach((project) => {
      const score = calculateComplianceScore(project.id);
      scores.set(project.id, score);
    });
    return scores;
  }, [projects, calculateComplianceScore]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 py-8">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-slate-900 mb-2">Compliance Scoreboard</h1>
          <p className="text-slate-600">
            Real-time project compliance assessment across documentation, milestones, budget, and audit metrics
          </p>
        </div>

        {/* Compliance Scoreboard Component */}
        <ComplianceScorebardComponent
          projects={projects}
...
interface ComplienceScorebardComponentProps {
  projects: any[];
  complianceScores: Map<string, any>;
}

const ComplianceScorebardComponent: React.FC<ComplienceScorebardComponentProps> = ({
  projects,
  complianceScores
}) => {
  ...
  const averageScore = complianceScores.size > 0
    ? Math.round(
      Array.from(complianceScores.values()).reduce((sum: number, s: any) => sum + s.totalScore, 0) /
  ...
  return (
    <div className="space-y-6">
      {/* Overall Score */}
      <div className={`${getScoreColor(averageScore).bg} border-2 border-slate-300 rounded-lg p-8`}>
